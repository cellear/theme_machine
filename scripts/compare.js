...
const LAST_RUN_PATH = 'reports/last-run.json';

if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log(`
compare.js — capture screenshots and watchdog output for D7 and Backdrop themes

Usage:
  node scripts/compare.js [options]

Options:
  --all              Run every theme in the THEMES list (default: first 10)
  --limit N          Run N themes (overrides default 10)
  --offset N         Start at position N in the THEMES list
...
Output:
  reports/comparison-{timestamp}.html   Side-by-side HTML report
  reports/last-run.json                 Theme list from the most recent run (used by build-reviewer.js)
  screenshots/{d7,backdrop}/{theme}/node.png
  screenshots/{d7,backdrop}/{theme}/meta.json
`);
  process.exit(0);
}
...
async function main() {
  ...
  // Write manifest for build-reviewer.js
  fs.writeFileSync(LAST_RUN_PATH, JSON.stringify({
    generatedAt: new Date().toISOString(),
    reportPath,
    themes: results.map(r => r.theme),
  }, null, 2));
  console.log(`Last-run manifest: ${LAST_RUN_PATH}`);
}
